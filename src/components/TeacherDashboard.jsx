...
export default function TeacherDashboard({ sessions, onAcceptSession, onDeclineSession, onStartEvaluation }) {
  ...
  const teacherSessions = sessions.filter(s => s.ustadzId === user?.id);

  const pendingRequests = teacherSessions.filter(s => s.status?.toLowerCase() === 'pending');
  const confirmedSessions = teacherSessions.filter(s => s.status?.toLowerCase() === 'confirmed');
  const completedSessions = teacherSessions.filter(s => s.status?.toLowerCase() === 'completed');
  
  // Stats calculations dynamically calculated from teacher's fetched real sessions
  const totalStudentsCount = [...new Set(teacherSessions.map(s => s.studentId))].length;
  const completedCount = completedSessions.length;
  const totalHoursTaught = completedCount > 0 ? (completedCount * 0.5).toFixed(1) : "0.0";
  const ratingAvg = completedCount > 0 ? "5.0" : "5.0";
    ...
    <div className="space-y-8">
      ...
      {teacherSessions.length === 0 ? (
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-12 text-center text-slate-400 space-y-3">
          ...
          <p className="text-base font-bold text-slate-800 m-0">No incoming requests or confirmed schedules at the moment.</p>
          <p className="text-xs text-slate-400 max-w-md mx-auto">
            Once students book recitation sessions with you, their requests and schedules will appear here in real-time.
          </p>
        </div>
      ) : (
